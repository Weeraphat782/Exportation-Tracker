...
function isDocumentQuestion(message: string, knowledge: string): boolean {
  if (!knowledge.includes('## Required export documents')) return false;
  return /document|paperwork|เอกสาร|ภ\.?\s*ท\.?|what do i need|required for export|ใช้เอกสาร/i.test(
    message
  );
}
