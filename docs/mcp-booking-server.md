...
1. Receive `quotation.created` webhook (or poll `list_new_quotations`)
2. `extract_booking_fields` — read Commercial Invoice first, then Export Packing List URLs
3. `update_quotation_net_weight` — persist correct net from CI/packing list (`source: "commercial-invoice"`)
4. `create_op_card` — create/link Opportunity (idempotent if already exists)
5. `build_booking_email_draft` — optional `net_weight_kg` override if not yet stored
6. Present draft to staff / copy to clipboard
7. `mark_booking_email_drafted` (idempotent)

MCP **never sends email**.
